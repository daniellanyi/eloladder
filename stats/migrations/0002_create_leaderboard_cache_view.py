...
SQL Queries:
    1. Create leaderboard_cache_view from the results
        #CREATE VIEW leaderboard_cache_view AS
    ...
    3. Order the rows primarily based on rank and secondarily based on the alphabetical value of the username.\n
        ...
        #RANK() OVER (ORDER BY s.elo DESC, l.username) AS player_rank
    
    4. Finish the process by selecting from an INNER JOIN of the stats_leaderboardcache and the stats_userstatsistics table.\n
        #FROM
                stats_leaderboardcache l
                INNER JOIN stats_userstatistics s ON l.user_id = s.user_id
